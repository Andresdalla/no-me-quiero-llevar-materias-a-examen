...
Transcripción literal de las cinco:
...
El apunte usa `R<` como contraejemplo de casi todas:

- "`R<` es reflexiva ? No, dado que `(0, 0) ∉ R<`"
- "`R<` es simétrica ? No, dado que `(0, 1) ∈ R<` pero `(1, 0) ∉ R<`"

"`R<` es de equivalencia ? No, por no ser reflexiva ni simétrica."

Quedan **dos preguntas sin responder** en el apunte: "`R<` es
