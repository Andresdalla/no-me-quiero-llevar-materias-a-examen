...
**Órdenes laxos.** ✅ [revision-conjuntos p.7] "Una relacion es un **orden parcial laxo** si
es: reflexiva · antisimétrica · transitiva."

✅ [revision-conjuntos p.8] "Es un **orden total laxo** si además cumple:
`(∀x, y ∈ A)((x, y) ∈ R ∨ (y, x) ∈ R)`"

**Órdenes estrictos.** ✅ [revision-conjuntos p.8] "Una relacion es un **orden parcial
estricto** si es: asimétrica · transitiva."

✅ [revision-conjuntos p.8] "Es un **orden total estricto laxo** si además cumple:
`(∀x, y ∈ A)((x, y) ∈ R ∨ (y, x) ∈ R)`"

⚠️ "orden total estricto **laxo**" está transcripto literal y se contradice a sí mismo: o es
...
✅ [revision-conjuntos p.8] "`R≤ = R< ∪ {(x, x) : x ∈ A}` es un orden total"
...
✅ [revision-conjuntos p.8] "`R<` no es un orden total porque no es reflexiva."

✅ [revision-conjuntos p.8] "`R<` no es un orden total estricto."

⚠️ Acá hay un problema real de la fuente: la condición de totalidad que da el apunte,
