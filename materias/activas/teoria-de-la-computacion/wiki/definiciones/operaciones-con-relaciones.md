...
**Relación inversa.** `R⁻¹ = {(x, y) : (y, x) ∈ R}`

**Composición.** "Sean `R ⊆ A × B` y `S ⊆ B × C`:"
...
Para funciones invierte la notación: la tabla pone `R ∘ S` del lado
de relaciones y `s ∘ r` del lado de funciones. Ver
[[comparativas/relaciones-vs-funciones]].

## Ejemplo

El apunte plantea tres ejercicios y **no los resuelve**:
`R⁻¹< = ?`, `R< ∘ R< = ?`, `R< ∘ R< ∘ R< = ?`
