...
✅ [repaso-haskell p.2] "Dado el siguiente data que represente árboles binarios con información
...
✅ [repaso-haskell p.2-3] La cátedra resuelve `cantNodos` en las dos formas:
...
✅ [repaso-haskell p.3] Firmas textuales del repartido:

| Ej. | Firma | Estado |
|---|---|---|
| (b) | `cantHojas :: Arb a -> Integer` | 🧠 resuelta arriba |
| (c) | `cantA :: Arb a -> Integer` | pendiente |
| (d) | `listA :: Arb a -> [a]` | ⚠️ pendiente, ver abajo |
...
⚠️ El repartido **no especifica el orden de recorrido de `listA`** (in-order, pre-order,
