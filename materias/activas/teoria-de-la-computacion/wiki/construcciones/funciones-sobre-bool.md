...
✅ [repaso-haskell p.1] "Recordemos el data de los booleanos visto en fundamentos:
`data Bool where {False :: Bool; True :: Bool}`"

✅ [repaso-haskell p.1] "Ahora veremos el data de la siguiente forma:
...
✅ [repaso-haskell p.1] La cátedra resuelve `not` en las dos formas:
...
✅ [repaso-haskell p.1] Firmas textuales del repartido:

| Ej. | Firma | Estado |
|---|---|---|
| (b) | `(&&) :: Bool -> Bool -> Bool` | 🧠 resuelta arriba |
| (c) | `(||) :: Bool -> Bool -> Bool` | pendiente |
| (d) | `(>>) :: Bool -> Bool -> Bool` | ⚠️ pendiente, ver abajo |
| (e) | `(==) :: Bool -> Bool -> Bool` | pendiente |

⚠️ **`(>>)` no está definido en el repartido.** El símbolo está verificado contra la página,
