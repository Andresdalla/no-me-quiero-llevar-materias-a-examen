...
✅ [numerabilidad-diag p.8] "Vamos a demostrar que el conjunto partes o potencia de `N`
(`P(N)` o `2^N`) no es numerable."

✅ [numerabilidad-diag p.9] Corolario que el apunte deja anotado: "Observar que, como se puede
ver en la tabla anterior, `P(N) ∼ N → Bool`."

## Hipótesis

- ✅ [numerabilidad-diag p.8] `P(N) = {A : A ⊆ N}`
...
✅ [numerabilidad-diag p.9] "Esta técnica se basa en suponer por absurdo `P(N)` es numerable,
...
✅ [numerabilidad-diag p.9] "A partir de esta tabla construimos un conjunto de naturales que
...
✅ [numerabilidad-diag p.9] "Con lo cual construimos un conjunto `D` de naturales tal que
...
✅ [numerabilidad-diag p.9] Los corolarios que el apunte deja como ejercicio `?19`: "Demostrar
