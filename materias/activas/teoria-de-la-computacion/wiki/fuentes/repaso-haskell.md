---
id: teoria-de-la-computacion/fuentes/repaso-haskell
tipo: fuente
tema: U10
...
# Ficha · Repaso de Haskell (repartido)
...
|---|---|---|
| Bool | `data Bool`, `not` resuelta, 4 operadores a definir | p.1 |
| Integer | `sumi` resuelta, 3 funciones de orden superior a definir | p.1 |
| Listas | `data [a]`, `length` resuelta, `map`/`filter`/`zip` a definir | p.2 |
| Listas con tipos | `sum`, `prod`, `and`, `or` a definir | p.2 |
| Árboles | `data Arb`, `cantNodos` resuelta, 4 funciones a definir | p.2-3 |
